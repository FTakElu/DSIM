...
# Script para configurar tabelas no DynamoDB Local
echo "Configurando DynamoDB Local..."

# Configura endpoint local
...
aws dynamodb put-item \
    --table-name Pacientes \
    --item '{
        "pacienteId": {"S": "pac-001"},
        "nome": {"S": "João Silva"},
        "dataNascimento": {"S": "1985-03-15"},
        "deviceId": {"S": "device123"},
        "cuidadorId": {"S": "cuidador-001"},
        "mewsLimits": {
            "M": {
                "hr_min": {"N": "60"},
                "hr_max": {"N": "100"},
...
# Dados da pulseira de exemplo
aws dynamodb put-item \
    --table-name DadosPulseira \
    --item '{
        "deviceID": {"S": "device123"},
        "timestamp": {"N": "1640995200000"},
        "heartRate": {"N": "75"},
        "temperature": {"N": "36.8"},
        "oximetry": {"N": "98"},
        "bloodPressureSystolic": {"N": "120"},
        "bloodPressureDiastolic": {"N": "80"}
    }' \
    --endpoint-url $ENDPOINT
